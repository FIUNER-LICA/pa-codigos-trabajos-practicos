...
from modules.formularios import FormRegistro, FormLogin, BooksForm
...
def funcion_listar(): 
    ...
    if request.args.get('seguir') == 'true':
        ...
        gestor_usuarios.registrar_libro_a_seguir(gestor_login.id_usuario_actual, id_libro)
    
    libros_seguidos = gestor_libros.obtener_libros_seguidos_por_usuario(gestor_login.id_usuario_actual)
    print(libros_seguidos)
    libros_existentes = gestor_libros.listar_libros_existentes()
...
def funcion_editar():
    ...
    if request.method == 'POST':
        try:
            gestor_libros.editar_libro(int(edit_form.id.data), 
                                       edit_form.nombre.data, 
                                       edit_form.autor.data, 
                                       float(edit_form.calificacion.data))
        except ValueError as e:
            flash(str(e))        
        else:
    ...
    return render_template("editar.html", libro=libro_a_editar, e_form=edit_form)
